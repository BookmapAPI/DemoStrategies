...
public class TrueStrengthIndexColorIndicator implements Layer1IndicatorColorInterface {
    ...
    public TrueStrengthIndexColorIndicator(TrueStrengthIndexRepo trueStrengthIndexRepo) {
    ...
    public Color getColor(String alias, String name) {
        ...
        if (color == null) {
            TrueStrengthIndexConstants indexColors = TrueStrengthIndexConstants.fromLineName(name);
            if (indexColors != null) {
                color = indexColors.getDefaultColor();
            } else {
                Log.warn("Layer1TrueStrengthIndex: unknown color name " + name);
                color = Color.WHITE;
            }
        }
    ...
    public IndicatorColorScheme createIndicatorColorScheme() {
        return new IndicatorColorScheme() {
            @Override
            public ColorDescription[] getColors() {
                return new ColorDescription[]{
                        new ColorDescription(Layer1TrueStrengthIndex.class,
                                TrueStrengthIndexConstants.MAIN_INDEX.getLineName(),
                                TrueStrengthIndexConstants.MAIN_INDEX.getDefaultColor(),
                                false),
                };
            }
            ...
            public String getColorFor(Double value) {
                return TrueStrengthIndexConstants.MAIN_INDEX.getLineName();
            }

            @Override
            public ColorIntervalResponse getColorIntervalsList(double valueFrom, double valueTo) {
                return new ColorIntervalResponse(
                        new String[]{TrueStrengthIndexConstants.MAIN_INDEX.getLineName()},
                        new double[]{});
            }
        };
    }
    ...
    private ColorsConfigItem createNewColorsLinesConfigItem(IndicatorColorInterface indicatorColorInterface) {
        ColorsChangedListener colorsChangedListener = () -> {
            InvalidateInterface invalidateInterface = trueStrengthIndexRepo.getInvalidateInterface(
        ...
        return new ColorsConfigItem(TrueStrengthIndexConstants.MAIN_INDEX.getLineName(),
                TrueStrengthIndexConstants.MAIN_INDEX.getLineName(),
                true,
                TrueStrengthIndexConstants.MAIN_INDEX.getDefaultColor(),
